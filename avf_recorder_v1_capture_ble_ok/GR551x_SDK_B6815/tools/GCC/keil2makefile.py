...
# Optional : "GR5515_C", "GR5515_D"
global g_target_cpu_model
g_target_cpu_model = "GR5515_D"
...
class MAKEFILE_TEMPLATE():
    g_makefile_template_target_cpu = \
    '''
TARGET_CPU := '''
...
ifeq ($(IS_WIN_OS),true)
	BLE_TOOL_BIN		= ../../../../../build/binaries/ble_tools/GCC/ble_tools_win_gcc.exe
else
...
## default make goal
all: mk_path $(BUILD)/$(MAKE_TARGET_APP).bin

##  compile C & asm files
%.o : %.c
	$(V_CC) $(CC) $(CFLAGS) -c $< -o $(BUILD_OBJ)/$(notdir $@)

# how to compile assembly files
%.o : %.s
	$(V_ASM) $(ASM) $(ASMFLAGS) -c $< -o $(BUILD_OBJ)/$(notdir $@)

## make depends
$(BUILD)/$(MAKE_TARGET_APP).bin: $(BUILD)/$(MAKE_TARGET_NAME).bin
	$(ECHO) "update binary file ..."
	$(BLE_TOOL_BIN) --cfg=../../../../../build/config/custom_config.h --mode=gen --bin=$(BUILD)/$(MAKE_TARGET_NAME).bin --outdir=$(BUILD) --app_name=$(MAKE_TARGET_APP)
	rm -rf $(BUILD)/info.* $(BUILD)/header.* $(BUILD)/*.tmp $(BUILD)/*.lds $(BUILD)/$(MAKE_TARGET_NAME).bin
	$(ECHO) "++++++++++++++++++++++++++++++++++++++++++"
	du -h $@
	$(ECHO) "++++++++++++++++++++++++++++++++++++++++++"
    
$(BUILD)/$(MAKE_TARGET_NAME).bin: $(BUILD_LST)/$(MAKE_TARGET_NAME).axf
	$(ECHO) "compile binary file ..."
	$(V_OBJCOPY) $(OBJCOPY) -O binary $< $@	

$(BUILD_LST)/$(MAKE_TARGET_NAME).axf: $(OBJ_ADJUST)
...
clean:
    ...
    def make(ProjectDict):
        ...
        MAKEFILE_TEMPLATE.__fill_single_value(mkfile, MAKEFILE_TEMPLATE.g_makefile_template_target_cpu, g_target_cpu_model)
        ...
        MAKEFILE_TEMPLATE.__fill_single_value(mkfile, MAKEFILE_TEMPLATE.g_makefile_template_target_app_key, ProjectDict["APP_NAME"] + "_app")
...
class KEIL5_PARSER():
    ...
    def __parse_target_groups(self, target_groups):
        ...
        for group in target_groups.findall("Group") :
            ...
            ## Workaroud : not add [gr_drivers] group files, because they are provided in sdk lib already!
            if (grp_name == "gr_drivers") and (g_target_cpu_model == "GR5515_C") :
                print(">>> ALERT: ignore group gr_drivers because it's provided in sdk lib ...")
                continue
...
if __name__ == "__main__":
    ...
    if (parm_keil_prj == None ) or (is_print_help == True):
        print(">>> Usuage: pyhon keil2makefile.py  *.uvprojx  [-d0] [-C/-D] [-h/--help]")
        print(">>>>>> Note 1 :  .py and *.uvprojx must in the same directory")
        print(">>>>>> Note 2 :  -d0 is optional, if specified, select the first found target to generate Makfile in default")
        print(">>>>>> Note 3 :  -C - specify target CPU to GR5515_C1;  -D - specify target CPU to GR5515_C4. GR5515_C4 is Default if not specified")
        print(">>>>>> Note 4 :  -gr5513 - specify gr5513 chip type")
        print(">>>>>> Note 5 :  -h or --help - print usuage help")
        sys.exit(0)

    if (is_target_cpu_gr5515_d == True) :
        g_target_cpu_model = "GR5515_D"
        which_target = 1
    else :
        g_target_cpu_model = "GR5515_C"
        which_target = 0
